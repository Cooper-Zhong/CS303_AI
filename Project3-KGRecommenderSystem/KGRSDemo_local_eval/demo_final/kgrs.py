...
class Dataloader:
    def __init__(self, train_pos, train_neg, kg_lines, train_batch_size: int = 128, neg_rate: float = 2):
        ...
        self.train_pos, self.train_neg = train_pos, train_neg  # train data
        self.n_user = max(list(set(self.train_pos[:, 0]) | set(
            self.train_neg[:, 0]))) + 1  # number of users
        self.n_item = max(list(set(self.train_pos[:, 1]) | set(
            self.train_neg[:, 1]))) + 1  # number of items
        self._load_ratings()
        self.known_neg_dict = []  # 保存目前已知的负的反馈样例
        self._add_recsys_to_kg()
        self.train_batch_size = train_batch_size
        self.neg_rate = neg_rate
        self.ent_num = self.n_entity + self.n_user  # total number of entities
        self.rel_num = len(self.rel_dict)

    def _add_recsys_to_kg(self):
        # Add the interaction data to the kg as the extra relation
        # create a new relation type for the interaction data "feedback_recsys"
        self.rel_dict['feedback_recsys'] = max(
            [self.rel_dict[key] for key in self.rel_dict]) + 1
    ...
    def _convert_kg(self, lines):
        ...
        kg = []
        rel_dict = {}
        # relation mapping to id
        for line in open('./relation2id.txt', encoding='utf8').readlines():
            elements = line.replace('\n', '').split('\t')
            rel_dict[elements[0]] = int(elements[1])  # film.film.star -> 0

        # kg data
        for line in lines:
            array = line.strip().split('\t')
            head = int(array[0])
            relation = rel_dict[array[1]]  # use the relation id
            tail = int(array[2])
            kg.append((head, relation, tail))  # 3-tuple
            entity_set.add(head)
            entity_set.add(tail)

        print('number of entities (containing items): %d' % len(entity_set))
        print('number of relations: %d' % len(rel_dict))
        # return kg, rel_dict, number of items
    ...
    def get_user_pos_item_list(self):
        ...
        for record in self.train_pos:
            user, item = record[0] - self.n_entity, record[1]  # 注意user的索引偏移回去了
            if user not in train_user_pos_item:
                train_user_pos_item[user] = set()
            # the items that user interacted with
            train_user_pos_item[user].add(item)
        item_list = list(set(all_record[:, 1]))  # all items in the kg
    ...
    def get_training_batch(self):
        ...
        for fact in pos_data + neg_data:
            ...
            # (head, relation) -> tail 对应一个物品（tail）的集合
            hr_tail_set[(fact[0], fact[1])].add(fact[2])
            # (relation, tail) -> head 对应一个用户或物品（head）的集合
            ...
            if sample_failed_time < sample_failed_max:
                for fact in self.kg:
                    if len(neg_data) >= len(self.kg) * self.neg_rate:
                        # If enough negative samples are generated, stop.
                        break
                    if random.random() > 0.5:
                        # Generate negative samples based on tail entities.
                        if fact[0] >= self.n_entity:
                            ...
                            while tail in hr_tail_set[(fact[0], fact[1])] and sample_failed_time < sample_failed_max:
                                ...
                                tail = random.randint(0, self.n_entity - 1)
                        if sample_failed_time < sample_failed_max:
                            # Add the generated negative sample to the list of negative data.
                            hr_tail_set[(fact[0], fact[1])].add(tail)
                            neg_data.append((fact[0], fact[1], tail))
                    else:  # Consider whether the header entity is user！
                        # Generate negative samples based on head entities.
                        if fact[0] >= self.n_entity:  # user
                            head = random.randint(
        ...
        pos_batches = np.array_split(pos_data, max(
            1, len(pos_data) // self.train_batch_size))
        neg_batches = np.array_split(neg_data, len(pos_batches))
        # Transpose batches to meet the model's input format.
...
class TransE(torch.nn.Module):
    def __init__(self, ent_num: int, rel_num: int, dataloader: Dataloader, dim: int = 64, l1: bool = True,
        ...
        self.device = torch.device('cuda:{}'.format(
            device_index)) if device_index >= 0 else torch.device('cpu')
        self.ent_num: int = ent_num  # number of entities
        self.rel_num: int = rel_num  # number of relations
        self.dataloader = dataloader
        self.dim: int = dim  # embedding dimension
        self.l1: bool = l1  # whether to use l1 norm
        self.margin: float = margin  # margin for the loss function
    ...
    def forward(self, head, rel, tail) -> torch.Tensor:
        ...
        # Calculate score, whether to use l1 norm
        if self.l1:
            score = torch.sum(torch.abs(torch.subtract(
                torch.add(head_emb, rel_emb), tail_emb)), dim=-1, keepdim=True)
        else:
            # score = torch.sum(torch.square(torch.subtract(
            # torch.add(head_emb, rel_emb), tail_emb)), dim=-1, keepdim=True)
            score = torch.sum(torch.pow(torch.abs(torch.subtract(
                torch.add(head_emb, rel_emb), tail_emb)), 1.5), dim=-1, keepdim=True)
        return -score  # Return negative score (for minimizing during training)
    ...
    def optimize(self, pos, neg):
        ...
        pos_matrix = torch.matmul(
            pos_score, torch.t(torch.ones_like(neg_score)))
        neg_matrix = torch.t(torch.matmul(
            neg_score, torch.t(torch.ones_like(pos_score))))
        # Calculate the loss using margin ranking loss and clamp it
    ...
    def ctr_eval(self, eval_batches: List[np.array]):
        ...
        for batch in eval_batches:
            rel = [self.dataloader.rel_dict['feedback_recsys']
                   for _ in range(len(batch[0]))]
            # User ID in the mixed KG should add the number of the entities in the origin KG (n_entity)
            score = torch.squeeze(self.forward(
                batch[0] + self.dataloader.n_entity, rel, batch[1]), dim=-1)
    ...
    def top_k_eval(self, users: List[int], k: int = 5):
        ...
        for user in users:
            # User ID in the mixed KG should add the number of the entities in the origin KG (n_entity)
            head = [
                user + self.dataloader.n_entity for _ in range(len(item_list))]
            rel = [self.dataloader.rel_dict['feedback_recsys']
            ...
            # Sort the score
            score_ast = np.argsort(
                scores.cpu().detach().numpy(), axis=-1)[::-1]
            sorted_items = []
    ...
    def train_TransE(self, epoch_num: int, output_log=False):
        ...
        for epoch in tqdm(range(epoch_num)):
            ...
            if output_log:
                print("The loss after the", epoch,
                      "epochs is", np.mean(losses))
...
class KGRS:
    def __init__(self, train_pos: np.array, train_neg: np.array, kg_lines: List[str]):
        ...
        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 2, "emb_dim": 16, "l1": True, "margin": 15,
                  "learning_rate": 0.003, "weight_decay": 0.00075, "epoch_num": 32}
        # 0.860 0.091   0.864 0.092
        # AUC: 0.692    NDCG5: 0.168

        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 2, "emb_dim": 16, "l1": True, "margin": 15,
                  "learning_rate": 0.003, "weight_decay": 0.0007, "epoch_num": 25}
        # AUC: 0.695  NDCG5: 0.171

        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 2, "emb_dim": 16, "l1": True, "margin": 15,
                  "learning_rate": 0.003, "weight_decay": 0.0007, "epoch_num": 28}
        # AUC: 0.696   NDCG5: 0.173

        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 2, "emb_dim": 128, "l1": True, "margin": 15,
                  "learning_rate": 0.003, "weight_decay": 0.0007, "epoch_num": 35}
        # AUC: 0.668   NDCG5: 0.10

        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 2, "emb_dim": 16, "l1": True, "margin": 15,
                  "learning_rate": 0.003, "weight_decay": 0.0007, "epoch_num": 35}
        # 0.862, 0.095   0.858 0.094
        # AUC: 0.6951   NDCG5: 0.183

        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 2, "emb_dim": 32, "l1": True, "margin": 15,
                  "learning_rate": 0.003, "weight_decay": 0.0007, "epoch_num": 30}
        # 0.870, 0.092  0.866， 0.095
        # AUC: 0.683   NDCG5: 0.159

        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 2, "emb_dim": 14, "l1": True, "margin": 15,
                  "learning_rate": 0.003, "weight_decay": 0.0007, "epoch_num": 30}
        # 0.849, 0.092  0.859, 0.093
        # AUC: 0.694    NDCG5: 0.178

        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 2, "emb_dim": 14, "l1": True, "margin": 30,
                  "learning_rate": 0.004, "weight_decay": 0.0007, "epoch_num": 30}
        # 0.845， 0.092
        # AUC: 0.6956  NDCG5: 0.170

        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 2, "emb_dim": 16, "l1": False, "margin": 30,
                  "learning_rate": 0.004, "weight_decay": 0.0007, "epoch_num": 30}
        # 0.832, 0.096  0.833 0.089

        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 3, "emb_dim": 16, "l1": True, "margin": 20,
                  "learning_rate": 0.003, "weight_decay": 0.0007, "epoch_num": 35}
        # 0.865, 0.093
        # AUC: 0.696

        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 3, "emb_dim": 32, "l1": True, "margin": 30,
                  "learning_rate": 0.003, "weight_decay": 0.0007, "epoch_num": 35}
        # 0.865 0.867
        # AUC: 0.688

        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 2, "emb_dim": 16, "l1": False, "margin": 30,
                  "learning_rate": 0.005, "weight_decay": 0.0007, "epoch_num": 30}
        # AUC: 0.667    NDCG5: 0.188
        ...
        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 4, "emb_dim": 16, "l1": True, "margin": 30,
                  "learning_rate": 0.003, "weight_decay": 0.0007, "epoch_num": 36}
        # 0.859
        # AUC: 0.690

        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 3.5, "emb_dim": 16, "l1": False, "margin": 30,
                  "learning_rate": 0.003, "weight_decay": 0.0007, "epoch_num": 36}
        # distance p=1.5 AUC: 0.689 NDCG5: 0.2

        config = {"batch_size": 256, "eval_batch_size": 1024, "neg_rate": 3.5, "emb_dim": 16, "l1": True, "margin": 35,
                  "learning_rate": 0.003, "weight_decay": 0.0007, "epoch_num": 36}
        # AUC: 0.704 NDCG5: 0.193
        # AC!!!!!!!
    ...
    def eval_ctr(self, test_data: np.array) -> np.array:
        ...
        """
        eval_batches = np.array_split(
            test_data, len(test_data) // self.eval_batch_size)
